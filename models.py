...
class AttributeCategory(Base):
    ...
    full_path: Mapped[str] = mapped_column(String(500), default="", nullable=False)
    external_key: Mapped[str] = mapped_column(String(128), default="", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, onupdate=utc_now, nullable=False)
...
class AttributeTemplate(Base):
    ...
    __table_args__ = (
        UniqueConstraint("category_id", "name", name="uq_attribute_templates_category_name"),
        Index("ix_attribute_templates_category_id", "category_id"),
    )
...
class AttributeTemplateField(Base):
    ...
    separator: Mapped[str] = mapped_column(String(16), default="/", nullable=False)
    ...
    __table_args__ = (
        ...
        Index("ix_attribute_template_fields_template_id", "template_id"),
    )
...
class AttributeAllowedValue(Base):
    ...
    value_type: Mapped[str] = mapped_column(String(32), default="value", nullable=False)
    is_global: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    is_recommended: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    source: Mapped[str] = mapped_column(String(64), default="import", nullable=False)
    sort_order: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, onupdate=utc_now, nullable=False)
...
class AttributeValueSynonym(Base):
    ...
    normalized_synonym: Mapped[str] = mapped_column(Text, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)

    allowed_value: Mapped[AttributeAllowedValue] = relationship("AttributeAllowedValue", back_populates="synonyms")

    __table_args__ = (
        UniqueConstraint("allowed_value_id", "normalized_synonym", name="uq_attribute_value_synonyms_normalized"),
    )
...
class AttributeTemplateRevision(Base):
    ...
    __table_args__ = (
        Index("ix_attribute_template_revisions_template_id", "template_id"),
    )
...
class AttributeBatch(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    template_id: Mapped[int | None] = mapped_column(ForeignKey("attribute_templates.id", ondelete="SET NULL"), nullable=True)
    ...
    input_mode: Mapped[str] = mapped_column(String(32), default="csv", nullable=False)
    ...
    __table_args__ = (Index("ix_attribute_batches_created_at", "created_at"),)
...
class AttributeProduct(Base):
    ...
    donor_sources: Mapped[list["AttributeDonorProductSource"]] = relationship(
        "AttributeDonorProductSource",
        back_populates="product",
        cascade="all, delete-orphan",
        order_by="AttributeDonorProductSource.priority",
    )

    __table_args__ = (
        Index("ix_attribute_products_batch_id", "batch_id"),
        Index("ix_attribute_products_model", "model"),
    )
...
class AttributeProductValue(Base):
    ...
    __table_args__ = (
        Index("ix_attribute_product_values_product_id", "product_id"),
        Index("ix_attribute_product_values_status", "status"),
    )
...
class AttributeProcessingLog(Base):
    ...
    __table_args__ = (
        Index("ix_attribute_processing_logs_batch_id", "batch_id"),
        Index("ix_attribute_processing_logs_product_id", "product_id"),
    )


class AttributeDonor(Base):
    __tablename__ = "attribute_donors"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    domain: Mapped[str] = mapped_column(String(255), nullable=False)
    base_url: Mapped[str] = mapped_column(Text, default="", nullable=False)
    selectors: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, onupdate=utc_now, nullable=False)

    mapping_rules: Mapped[list["AttributeMappingRule"]] = relationship(
        "AttributeMappingRule",
        back_populates="donor",
        cascade="all, delete-orphan",
    )
    sources: Mapped[list["AttributeDonorProductSource"]] = relationship(
        "AttributeDonorProductSource",
        back_populates="donor",
    )

    __table_args__ = (
        UniqueConstraint("domain", name="uq_attribute_donors_domain"),
        Index("ix_attribute_donors_name", "name"),
    )
...
class AttributeMappingRule(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    donor_id: Mapped[int] = mapped_column(ForeignKey("attribute_donors.id", ondelete="CASCADE"), nullable=False)
    template_id: Mapped[int | None] = mapped_column(ForeignKey("attribute_templates.id", ondelete="CASCADE"), nullable=True)
    template_field_id: Mapped[int] = mapped_column(ForeignKey("attribute_template_fields.id", ondelete="CASCADE"), nullable=False)
    donor_attribute_name: Mapped[str] = mapped_column(String(255), nullable=False)
    normalized_donor_name: Mapped[str] = mapped_column(String(255), nullable=False)
    confidence: Mapped[int] = mapped_column(Integer, default=100, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, onupdate=utc_now, nullable=False)

    donor: Mapped[AttributeDonor] = relationship("AttributeDonor", back_populates="mapping_rules")
    template: Mapped[AttributeTemplate | None] = relationship("AttributeTemplate")
    ...
    __table_args__ = (
        ...
        Index("ix_attribute_mapping_rules_template_id", "template_id"),
    )


class AttributeDonorProductSource(Base):
    __tablename__ = "attribute_donor_product_sources"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    product_id: Mapped[int] = mapped_column(ForeignKey("attribute_products.id", ondelete="CASCADE"), nullable=False)
    donor_id: Mapped[int | None] = mapped_column(ForeignKey("attribute_donors.id", ondelete="SET NULL"), nullable=True)
    url: Mapped[str] = mapped_column(Text, nullable=False)
    priority: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    raw_html_path: Mapped[str] = mapped_column(Text, default="", nullable=False)
    ...
    product: Mapped[AttributeProduct] = relationship("AttributeProduct", back_populates="donor_sources")
    donor: Mapped[AttributeDonor | None] = relationship("AttributeDonor", back_populates="sources")

    __table_args__ = (
        Index("ix_attribute_donor_product_sources_product_id", "product_id"),
    )
