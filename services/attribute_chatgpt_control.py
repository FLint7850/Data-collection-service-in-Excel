"""Authentication controls for the isolated Codex App Server bridge."""
...
def bridge_request(method: str, path: str, timeout: int = 30) -> dict[str, Any]:
    session = requests.Session()
    session.trust_env = False
    headers = {"Authorization": f"Bearer {BRIDGE_TOKEN}"} if BRIDGE_TOKEN else {}
    try:
        response = session.request(method, f"{BRIDGE_URL}{path}", headers=headers, timeout=timeout)
    except requests.RequestException as error:
        message = clean_text(str(error)) or "Codex bridge ??????????"
        raise RuntimeError(f"Codex bridge ??????????: {message}") from error
    try:
        data = response.json()
    except ValueError:
