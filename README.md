...
### Прокси для парсеров и Codex

Парсеры сайтов и Codex используют один явный HTTP(S)-прокси для внешних адресов.
Загрузка XML/CSV-фидов, сравнение фидов и внутренние обращения между Flask, Nuxt,
Nginx, localhost и приватными сетями идут напрямую. Настройка хранится только в
локальном `.env`:
